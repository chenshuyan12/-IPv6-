...
////////////
  ...
  int nExTimes;
  int pExTimes;
  struct ExInfo *nExInfo;//这个100还有待改进，先这样
  struct  ExInfo *pExInfo;
  void trainFgpu(const std::vector<std::pair<std::vector<int>,int> >& ferns,int resample);
  int  getnExPosition();
  int  getpExPosition();
////////////
  float thr_nn_valid;  
